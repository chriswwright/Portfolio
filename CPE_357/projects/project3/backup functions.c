...
void decotree(int file){
   ...
   head->right = NULL;
   head->frequency = 0;
   num = read(file, sbuf, 8);
   if(num != 0){
     ss[0] = sbuf[0];
     ss[1] = sbuf[1];
     size = (int)strtol(ss, &p, 16);
   }
   
   while(size != 0){
      num = read(file, buf, 10);
      size--;
      ss[0] = buf[0];
      ss[1] = buf[1];
      hi = strtol(ss, &p , 16);
...
char* decoder(int file, char* argv[]){
   ...
   int hex = 0;
   while(num == 1){
      num = read(file, sbuf, 1);
      if (num == 1){
      strcat(buf, sbuf);}
   }
   for(i = 0; buf[i]!= 0; i++){if (buf[i]== 10){buf[i] = '\0';}}
   sbuf = NULL;
   sbuf = malloc(4);
   for(i = 0; buf[i]!= 0; i++){
     ss[0] = buf[i];
     hex = (int)strtol(ss, &p, 16);
     sprintf(sbuf, "%d%d%d%d",(hex/8)%2,(hex/4)%2,(hex/2)%2,hex%2);
